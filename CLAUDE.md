...
| `tests/`, `package.json` | Headless regression harness (`npm test`), dev-only `playwright-core`. `manual.test.js` reads the numbers back out of the two source books and compares them to the data files, so a table that drifts from print fails the build | **present** |
...
| `now.js` | **New module:** the solo "Now" bar — one line above every panel saying what to do next, and the button that does it. A pure resolver over state the app already keeps (a filled clock, a crossed suspicion threshold, the raid, the scene, the last answer) plus the renderer both Solo and Roll use | alerts outrank the loop step; the bar follows to the Roll screen while a scene is open |
| `solo.js` | Oracle, Random Event, Meaning/Element tables, solo loop | enabled (official rules); the screen is laid out in §23 loop order — frame · ask · resolve · track · suspicion · close the scene · read back, with the Now bar pinned above it. It fires the End Scene boundary itself, and an Oracle answer can feed a clock |
...
- [x] **Solo usability pass — the "Now" bar** (`src/now.js`): one instruction pinned above the panels, alerts first, acting where it stands
...
|---|---|---|---|---|
| 2026-08-12 | **The solo tab tells you what to do next.** The reported fault was never "I cannot find the control", it was **"I do not know what to press"**: eight panels, twenty controls and 377 words at 390px, all of them equally available, with the printed loop's own numbering (`1 ·`…`6 ·`) implying you walk it top to bottom when in practice you re-enter it wherever the last thing left you. New **`src/now.js`** (added to the §7.1 map and the service-worker shell) resolves one question — *what now?* — from state the app already keeps, and renders it as a bar pinned above every panel: a label, one line, one button that **acts where it stands** rather than pointing at a panel further down. Nine states, first match wins, and **alerts outrank the loop step**, because a clock that has run out or a threshold that has just come into force changes the fiction under you and the next question would otherwise be asked in a world you have not noticed: a filled clock (§8A) · a newly-crossed suspicion threshold, named by its effect and acknowledged once (§17.2) · the raid the Oracle now decides (§23) · no scene running · a scene with nothing asked · an answer that fired a random event · an answer with something attached, which hands off to the Roll screen · a plain yes or no, which asks the next question · and, on the Roll screen itself, the scene you are still in and the way back. **The bar follows to the Roll screen** while a solo scene is open, so the hand-off is not a dead end; the two alerts act there too, since they need nothing but their own state. The panel headings **lose their numbers** — the bar carries the sequence now, and the panels become the place to change a setting rather than the place to find out what to do. `askNow` and `askRaid` are extracted so the bar and the panels run the same act rather than two copies of it, and the Oracle's last answer now records its rung and whether an event fired. **Also fixed:** the boot-time screen check sampled `#screen .card` seven times without waiting, so the service-worker handshake re-rendering underneath it made two of those samples flake. | User: the whole solo system is not idiot-proof or intuitive enough — research how to fix the layout | `npm test`: **911 checks pass, zero console errors**, and the solo screen measured at 390px: 2,725px → 2,914px, 8 panels, 21 controls (the bar adds one). New checks drive every branch — the bar is the first thing on the screen, names the scene, asks in place without scrolling, and the precedence is exercised on a single answer that both fired an event and crossed a threshold: the threshold is announced first, acknowledging it hands the bar back to the event, the same threshold is never announced twice, and the bar is asserted present on the Roll screen with a way back | `reich62-v55` |
| 2026-08-11 | **Interface and conformance audit: every button clicked, every printed number compared.** Two sweeps. *(1) Buttons:* a headless pass opened all ten screens plus the seven sheet sub-tabs and five GM sub-tabs with every accordion forced open, then clicked **278 controls** one at a time from a fresh page load, snapshotting storage, DOM and route around each — followed by a second pass driving the creation wizard through all nine steps for another **58**. **336 buttons, every one acted; no dead controls, no runtime errors.** Three wizard Motivation "Roll" buttons first read as no-ops; driving `rollMotivation` 60 times per facet showed all 10 printed entries reachable and assigned, so the flag was the sweep comparing `innerHTML.length` and missing a same-length swap. *(2) Conformance:* a new **`tests/manual.test.js`**, wired into `npm test`, parses `source/reich62_manual.md` and `source/reich62_bestiary.md` and compares **70 printed tables and values** against the data files — the difficulty ladder's dice counts, ranged difficulty by band, the falling table, base thresholds, all four XP costs, Push's cost and prices, §8A's sizes and the Triumph rule, all 29 Critical Injury bands by name and severity with contiguity to 151+, all 27 item qualities with their active/passive type, all 71 talents by name with the tier split and the book's own 11 setting-flex marks, all 11 careers with their 8 skills, the rarity ladder, the item-damage repair costs, all 10 weapons, 6 armours and 17 vehicles field by field, the suspicion ladder and safehouse bands, the Oracle pools, every solo and Part V table's size, and the bestiary's block counts and dragnet escalation. **A parsing bug in the audit itself was worth keeping:** the manual writes negative handling with the Unicode minus, which a plain `/-?\d+/` reads as positive — the check now normalises it, so a sign error in a vehicle table could never pass unnoticed. | User: audit every button and check the system matches the manual | `npm test`: **897 checks pass, zero console errors** (827 before, plus the 70 conformance checks). The button sweeps and the 228-check backend sweep ran alongside | `reich62-v54` |
| 2026-08-11 | **Backend audit: every exported function in every `src/` module exercised, one real defect found.** A 228-check sweep drove the engine layer directly rather than through the screens — every lookup over its whole table (26 skills, 71 talents, 11 careers, 10 weapons, 6 armours, 17 gear, 17 vehicles, 27 qualities, 16 conditions, 28 bestiary entries, 4 encounter blocks), the Critical Injury table across rolls 1–200, the trauma table across 1–100, every rarity 0–14, both suspicion models side by side, every clock tick rule in both directions, every lifecycle boundary fired and undone, every vehicle added and given a trait, every hooked talent used, every spend-table row applied, and the whole creation loop from career to a saved character. **Found:** 15 §12B Motivation entries — Death, Adaptable, Courageous and the other bare-word facets — reached the rules library with an **empty body**, so the row rendered as a title above a blank line. The index now falls back to naming the facet. Everything else the sweep flagged was the audit calling a function with the wrong signature, corrected in the sweep rather than in the app. | User: audit the backend thoroughly | 228/228 backend checks pass with zero runtime errors, alongside `npm test`'s **827 checks, zero console errors** | `reich62-v53` |
