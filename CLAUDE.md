...
- **Focus (H-2, house aid):** a d100 alongside every question, reading the answer against
  what the player expected. Ten bands; the chaos band resolves on a d10 against the higher
  Heat track doubled; 96–100 chains an event and rerolls. Layered on the printed Oracle, not
  a replacement, and switchable off.
...
| **H-2** | **House aid, supplied by the table owner from another solo system, in neither book:** the printed Oracle (§18) answers a question yes or no, and nothing reads that answer against what the player was expecting when they asked, so every answer lands flat | A **d100 focus roll** accompanies every Oracle question and says how to read the answer: as you expected · not quite · a surprise · in your favour · against you · let suspicion decide · a turn in the story · as expected but… · as expected and… · something else happens. The "let suspicion decide" band rolls a d10 against **the higher of Personal and Cell Heat, doubled** — at or under it the answer works against you, above it in your favour. A 96–100 chains one event and rerolls for the focus itself; a second 96–100 reads as what you expected. **One event per question**, whether the trigger was the focus or an emphatic R-22 answer. Paraphrased throughout, never presented as printed, and switchable off in Settings | in prose | `data-solo.js` `FATE_FOCUS`, `solo.js` `rollFocus`/`focusChaos`, `settings.js` |
...
- [x] **T69** **Fate Question Focus (10 d100 bands)** — H-2 *(house aid, in neither book; paraphrased, labelled, and layered on the §18 Oracle)*
