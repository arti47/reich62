...
- **Named clocks (H-4, house aid)** — the Dragnet's shape generalised: any track with a size
  and a direction, ticked off the symbols a resolved check already produced. The two
  published tracks (Dragnet, Heat) keep their printed behaviour and are never ticked by it.
...
| **H-4** | **House aid, in neither book:** the manual publishes no progress-clock subsystem (§3.13); the bestiary publishes exactly one extended track, the Manhunt/Dragnet (B§6), and Heat (§17) is a 0–5 track by another name. Anything else a player wants to see coming — a rival closing in, an internal review, a forgery nearing completion — has nowhere to live | **Named clocks:** a title, a size (4/6/8) and a direction — closing on you, or working for you. They are ticked by **the symbols the check already produced**, so no new dice and no second economy: each uncancelled 🔻 fills one segment of a clock closing on you and an uncancelled ⚡ fills two; each uncancelled 🌟 fills one of a clock you are working on and every two 🔺 fills one more; an uncancelled ☀️ fills yours or clears one of theirs. A clock that fills has arrived. The Roll screen gains one control — *does this feed a clock?* — and every move records why. The two published tracks stay exactly as printed: the Dragnet keeps its own escalating opposition and dual Heat cost, and Heat keeps its printed rates and thresholds; neither is ticked by this | in prose | `data.js` `CLOCKS`, `clocks.js`, `roller.js`, `combat.js`, `solo.js` |
...
| `solo.js` | Oracle, Random Event, Meaning/Element tables, solo loop | enabled (official rules); the screen is laid out in §23 loop order — frame · ask · resolve · track · read back |
...
campaigns/{campaignId}
  ...
  tasks/{taskId}: { name, kind: "clock"|"dragnet", direction: "against"|"for",  // H-4, B§6
