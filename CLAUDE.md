...
| **H-3** | **House rule, in neither book:** the manual assumes the PC is hiding from the regime — Heat reads as "how closely the regime is watching you", and every checkpoint, papers check and dragnet is aimed at the character. An SD or Gestapo PC, or one with no side, needs the same track to mean something else | Each character and each cell carries an **allegiance**: *against the regime* (the printed seat) · *inside the regime* · *no side but your own*. It changes nothing numeric — the levels, dice, decay and escalation are all as printed — but it **relabels the §17.3 ladder** (level 2 reads "credentials queried" rather than "papers checked on sight", level 5 "your own service moves to arrest you"), renames what gets blown (safehouse · station · bolt-hole), rewords the roller's surveilled toggle, and **decides who is hunting whom**: Papers-Check Reflex and the regime's own blocks do not fire against a character inside the regime. Career sets it and the player can move it; a mixed party is flagged on the GM screen | in prose | `data.js` `ALLEGIANCES`/`CAREER_ALLEGIANCE`, `rules.js` `heatThresholdsFor`, `heat.js`, `combat.js` `regimeBlocksApply`, `wizard.js`, `gm.js` |
...
campaigns/{campaignId}
  ...
  cell:    { name, allegiance, cellHeat: 0-5, safehouseStatus: "clear"|"watched"|"blown" } // §3.8, H-3
...
characters/{characterId}
  owner, campaignId
  identity:  { name, career, allegiance: "opposed"|"regime"|"unaligned",            // H-3
               careerSkills[4], motivation:{desire,fear,strength,flaw},
