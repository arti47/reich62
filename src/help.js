...
export const PANELS = {
  ...
  soloTables: { lede: 'Start the scene, and roll a prompt when you need one: a phrase, a place, a faction, a complication, a stranger, an encounter.', detail: 'Starting a scene is bookkeeping rather than a rule — it gives the scene a number and a name so the closing boundary has something to close, and so the app can say which scene you are in. The tables are the book\'s own, rolled for you; the last one you rolled stays here and the rest are kept under "What has happened".' },
