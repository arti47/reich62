...
export const PANELS = {
  ...
  soloScene: { lede: 'Open a scene: who noticed you, what suspicion is doing to you, and where you are.', detail: 'Passive Watch is the informant network\'s standing roll from the bestiary — the more suspicion on you, the likelier it is that someone reports you. Nothing it turns up moves your suspicion on its own; the books name no number, so the rise is offered rather than taken. The encounter table is kept on its own button so a quiet scene can stay quiet.' },
