...
export const PANELS = {
  ...
  clocks: { lede: 'Anything you want to see coming: name it, size it, and let your rolls fill it.', detail: 'A house aid, not a printed rule — the books track only the dragnet this way. A clock closing on you fills on leftover Threat, two segments on a Despair; a clock you are working on fills on leftover Success, and one more for every two Advantage. A Triumph fills yours or clears one of theirs. Point a check at a clock on the Roll screen and the symbols do the rest. The manhunt from the bestiary is a clock too, and keeps its own printed escalation and suspicion cost.' },
  soloTables: { lede: 'Prompts when you need one: a phrase, a place, a faction, a complication, a stranger, an encounter.', detail: 'These are the book\'s own tables, rolled for you. Roll one to open a scene when you do not know what happens next; everything you roll is kept below under "What has happened".' },
  ...
  soloScene: { lede: 'Close the scene when it has played out, then frame the next one.', detail: 'Ending a scene expires scene-length effects and re-checks every suspicion threshold. It is the same boundary the combat tracker fires, so it counts whether you end it here or there, and it can be undone once.' },
