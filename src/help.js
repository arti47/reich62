...
export const PANELS = {
  ...
  homeCell: { lede: 'Your network at a glance: how hot it is, whether the safehouse is safe, and the story-point bank.', detail: 'Cell suspicion is shared by everyone in the group. It climbs when a member\'s own suspicion reaches 3, and it drives whether the safehouse is clear, watched or blown (§17.2).' },

  sheetVitals: { lede: 'Injury, stress and suspicion. Nudge them as play goes.', detail: 'Injury and stress each have a limit; reaching either takes you out of the fight until you drop back below it. Type a number straight in, or use the buttons for small changes (§6).' },
  sheetSkills: { lede: 'What your character is good at, and the dice each skill rolls.', detail: 'The pool preview shows how many plain and upgraded dice you get: the higher of skill and its characteristic sets the number of dice, and the lower upgrades that many of them (§2).' },
  sheetGear: { lede: 'What you are carrying, what it weighs, and what state it is in.', detail: 'Going over your carrying limit adds a penalty die to physical checks, and going far over costs you your free move each turn. Items can be damaged and repaired a step at a time, and larger items have slots for attachments (§5F, §14B, §14C).' },
  sheetTalents: { lede: 'Your talents. Tap Use on the ones that do something on demand.', detail: 'Using a talent deducts its cost — stress or a story point — and marks it as spent for the encounter or session where the book says so (§12A).' },
  ...
  sheetCriticals: { lede: 'Lasting injuries. Each untreated one makes the next roll on the injury table worse.', detail: 'Every untreated injury adds ten to future rolls on that table, so they stack up fast. Treating one takes a Medicine check or a week of rest (§5G, §9).' },
  ...
  sheetRecovery: { lede: 'Healing, and the limits on how often each kind can be used.', detail: 'The book puts a hard limit on most of these — once per encounter, once a night, once a week per injury — and the app enforces them rather than trusting you to remember (§5G).' },
  sheetAdvance: { lede: 'Spend earned experience on skills and talents.', detail: 'Characteristics can only be raised during creation; afterwards only the Dedication talent raises one, never above 5 and never the same one twice. Talents follow the pyramid: you need as many talents in the tier below as you are about to have in the tier above (§7, §12A).' },

  rollCheck: { lede: 'Pick the skill and how hard the task is. The pool builds itself.', detail: 'Choose "opposed" when another person is actively resisting you: they never roll, their rating builds the opposing dice instead (§3A).' },
  rollSituation: { lede: 'Anything about the scene that should change the dice.', detail: 'Cover, darkness, the size of your target and how hard the target is to hit all belong here. Below that you can upgrade or downgrade dice by hand, or spend a story point to do it (§5E, §5J, §2.4, §8).' },
  rollPool: { lede: 'The dice this check uses, and why each one is there.', detail: 'Dice are assembled in the order the book gives: build the base pool, add, upgrade, downgrade, then remove (§2.4).' },
  rollEntry: { lede: 'Roll your physical dice, then tap what came up.', detail: 'The book never prints what is on each die face, so the app treats your physical dice as the source of truth. Face data was supplied separately, so you can also let the app roll for you — switch that on in Settings.' },
  rollResult: { lede: 'The outcome, what you can spend leftover symbols on, and what it did to your suspicion.', detail: 'Successes and failures cancel each other, and so do advantages and threats. The best and worst results never cancel and always happen (§1).' },
  rollLog: { lede: 'The last hundred checks, with enough detail to re-derive any of them.', detail: 'Kept on this device only. It records the pool, what you entered, the net result and any suspicion change.' },

  combatLifecycle: { lede: 'End an encounter, scene, session, day, week or adventure. Each one runs its whole bundle of effects.', detail: 'The app shows exactly what will change before it changes anything, and every boundary can be undone once. Ending a session is where experience is awarded and suspicion cools (§21–§24).' },
  combatInitiative: { lede: 'Turn order. Slots belong to a side, not to a person.', detail: 'Everyone rolls once at the start. That produces an ordered list of slots, each owned by either the players or the GM. Each round, whoever owns the slot picks which of their people takes it — so the order of your own turns is yours to choose (§5A\').' },
  combatRoster: { lede: 'Everyone in the fight. Drop opponents in from the bestiary.', detail: 'Groups of minions share one injury pool and drop a member at a time; named opponents take injuries normally. Published opponents load exactly as printed (§12C).' },
  combatVehicles: { lede: 'Cars, bikes, trucks and trains, on the same engine as everything else.', detail: 'Speed changes by one step at a time. Losing control crashes the vehicle for damage equal to its current speed (§12).' },
  combatTasks: { lede: 'Anything that takes several rolls: a manhunt, a repair job, or a clock you invent.', detail: 'The manhunt is the one the books actually publish: the search gets stronger every hour, and each failed round raises suspicion on both you and your network (B§6).' },

  soloOracle: { lede: 'Ask a yes-or-no question when nothing on your sheet decides it.', detail: 'Set how likely the answer is, roll the dice listed, and enter what came up. A best or worst result also triggers a random event (§18, §19).' },
  soloTables: { lede: 'Prompts when you need one: a phrase, a place, a faction, a complication, a stranger, an encounter.', detail: 'These are the book\'s own tables, rolled for you (§15A, §15B, §19, §20).' },

  gmCell: { lede: 'The network\'s suspicion and its story-point bank.', detail: 'Raising cell suspicion here changes the safehouse status automatically: watched at 3, blown at 5 (§17.2, §17.3).' },
  ...
  gmEncounters: { lede: 'Whole encounters reduced to a single check, ready to drop on the table.', detail: 'Each one names the skills involved, how strong the opposition is and what failure costs (B§6).' },
  ...
  gmBuild: { lede: 'Make an opponent from scratch using the book\'s recipes.', detail: 'Opponents built here derive their numbers from the recipes, and are stored separately from published ones so you can always tell which is which (§12C).' },
